...
/**
 * YouTube InnerTube API로 검색 + lrclib 가사 처리.
 * 재생은 YouTubeExtractor(NewPipeExtractor)가 담당.
 */
class ApiService {
    ...
    fun searchYouTube(query: String): JSONArray {
        return try {
            ...
            android.util.Log.e("XWare/API", "searchYouTube: ${e.message}")
            JSONArray()
        }
    }
    ...
    private fun searchLrc(q: String): JSONArray {
        return try {
            val resp = client.newCall(
                Request.Builder()
                    .url("https://lrclib.net/api/search?q=${enc(q)}")
                    .header("Lrclib-Client", "XWare/1.0")
                    .build()
            ).execute()
            val json = resp.body?.string() ?: return JSONArray()
            resp.close()
    ...
    private fun parseLrc(lrc: String): JSONArray {
        ...
        list.forEachIndexed { i, l ->
            ...
            result.put(JSONObject().apply {
                put("start", l.start); put("end", end); put("text", l.text)
            })
        }
        return result
    }
