...
class AudioPlayerService : Service() {
    ...
    private fun load(videoId: String) {
        ...
        dbg("🔍 NewPipeExtractor 추출 시작: $videoId")

        scope.launch(Dispatchers.IO) {
            val info = runCatching { extractor.extractAudio(videoId) }.getOrElse {
                dbg("❌ 추출 예외: ${it.message}")
                null
            }
            ...
            withContext(Dispatchers.Main) {
                ...
                dbg("✅ URL 획득 (${info.audioUrl.take(60)}...)")
                ...
                player = MediaPlayer().also { mp ->
                    ...
                    mp.setOnErrorListener { _, what, extra ->
                        dbg("❌ MediaPlayer 오류: what=$what extra=$extra")
                        onError?.invoke(what)
                        stopTick()
                        true
                    }
    ...
    private fun doPlay()  { runCatching { player?.start(); onStateChanged?.invoke("playing"); startTick(); updateNotification() } }
    private fun doPause() { runCatching { player?.pause(); onStateChanged?.invoke("paused");  stopTick();  updateNotification() } }
